...
"
    ...
    # Generate requirements.txt alongside the .supx from pyproject.toml [project.dependencies]
    local req_file="${SCRIPT_DIR}/${publisher}.${ext_pkg_name}-${version}-requirements.txt"
    python3 -c "
import sys
try:
    import tomllib
...
with open('${ext_dir}/backend/pyproject.toml', 'rb') as f:
    cfg = tomllib.load(f)

deps = cfg.get('project', {}).get('dependencies', [])
if deps:
    with open('${req_file}', 'w') as out:
        out.write('# Auto-generated by build-extensions.sh from ${ext_name}/backend/pyproject.toml\n')
        out.write('# Copy these lines into docker/requirements-local.txt before deploying.\n')
        for dep in deps:
            out.write(dep + '\n')
    print(f'  Requirements: ${req_file}')
else:
    print('  No extra Python dependencies')
"
